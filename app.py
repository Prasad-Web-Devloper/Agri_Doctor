...
mydb = mysql.connector.connect(host="localhost", user="root", password="", port=3306,  database="e-agri")
cursor = mydb.cursor()

#email logins
sender_password = "vvyecubkjtkiqmok"
sender_email = "prasadyadavakunta@gmail.com"
...
def cropdata():
    ...
    x=x.drop(['cyear','investment','profit', 'area'], axis=1)
...
def funds():
    ...
    x = pd.read_sql_query(sql, mydb)
    x=x.drop(['status', 'e_money','crop_price'], axis=1)
...
def investback():
    if request.method=='POST':
        ...
        if m < invest:
            ...
            session1.starttls()
            session1.login(sender_email, sender_password)
            text = message.as_string()
            session1.sendmail(sender_email, receiver_address, text)
            session1.quit()
            ...
            session2.starttls()
            session2.login(sender_email, sender_password)
            text1 = message1.as_string()
            session2.sendmail(sender_email, receiver_address1, text1)
            # session2.quit()
...
def accept(s=0,s1="",s2="",s3=0, s4 = 0):
...
@app.route('/reject/<s>/<s1>/<s2>/<s4>')
def reject(s=0,s1="",s2="", s4 = ""):
    name=session.get('name')
    print(name)
...
def forgetback():
    if request.method == "POST":
        ...
        if count==0:
            ...
            ses.starttls()
            ses.login(sender_email, sender_password)
            text = message.as_string()
            ses.sendmail(sender_email, receiver_address, text)
            ses.quit()
